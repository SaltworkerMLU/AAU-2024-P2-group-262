classdef UR5
    ...
    methods (Static)
        ...
        function oritentation = sixDOF(TBW)
            ...
            oritentation = [TBW(1,4), TBW(2,4), TBW(3,4), 180/pi*gamma, 180/pi*beta, 180/pi*alpha];
        end
        
        % Get transform matrix T of 6DOF position (xyz gamma betaalpha)
        % Function makes use of the XYZ angle-set convention
        function pose = transformMatrix(DOF)
            % Rotation matrix
            pose = rotz(pi/180*DOF(6)) * roty(pi/180*DOF(5)) * rotx(pi/180*DOF(4));
        ...
        function solutions = inverseKinematics(T06)
            ...
            for i = 1:8 % For all 8 solutions
                ...
                if ~isreal(Joint(i,1)) || ~isreal(Joint(i,3)) || ~isreal(Joint(i,5)) 
                    Joint(i,:) = NaN; % ... remove solution ...
                    continue % ... and skip solution
                end
            ...
            Joint = Joint * 180/pi; % Convert to degrees
            Joint = unique(Joint,'rows'); % Remove duplicate solutions

            for i = 8:-1:1 % Countdown...
                if isnan(Joint(i))
        ...
        function tf = moveJtf(P0, Pf, v_max, a_max)
            ...
            if 2*tb > tf_a % If velocity does not reach max
                tf = tf_a;
            else % Otherwise, if velocity reaches max
                tf = tf_v + tb;
            end
        ...
        function moveJ(P0, Pf, v_max, a_max)
            ...
            timeInterval = 0:tf; %Time interval displayed using the method % 0:timeStep=1:UR5.moveLtf(P0, Pf)
            ...
            % Displays UR5 movement in runtime
            for i=1:length(timeInterval)
                configNow=a0+a1*timeInterval(i)+a2*timeInterval(i)^2+a3*timeInterval(i)^3; % Aquire position & % jointConfigArray(:,i)
                poseNow = UR5.forwardKinematics(configNow, 1, 6); % Transpose configNow
                ...
                %configNow(1) = configNow(1); % An error in MabLab plotting system accounted for
                show(ur5_RBT,configNow,'PreservePlot',false,'Frames','off');
                plot3(-poseNow(1,4),-poseNow(2,4),poseNow(3,4),'r.','MarkerSize',20); % jointSpaceMarker = 
        ...
        function moveL(P0, Pf, v_max, a_max)
            ...
            TBS = UR5.forwardKinematics(P0, 1, 6); %UR5.TB0 \ poseNow / UR5.T6W
            ...
            for i = 1:3 % mm -> m
                TBS(i,4) = TBS(i,4) / 1000; 
                TBE(i,4) = TBE(i,4) / 1000;
            end

            TSE = TBS \ TBE; % TSE = TBS \ TBE; %Compute TSE (Angle-axis representation)
            theta = acos((TSE(1,1)+TSE(2,2)+TSE(3,3)-1)/2); %Find axis angle
            Ps = [0 0 0 0]; % Start frame position (origo)
            Pe = [TSE(1,4) TSE(2,4) TSE(3,4) theta]; % end postion relative to start frame
            ...
            [~, testVel] = transformtraj(TBS, TBE, [timeInterval(1); timeInterval(end)], 0);
            [tTask,stateTask] = ode15s(@(t,state) derivative(tsMotionModel, state, transformtraj(TBS, TBE, [timeInterval(1); timeInterval(end)], t), testVel),[timeInterval(1); timeInterval(end)],[P0, zeros(size(P0))]);
            ...
            % Displays UR5 movement in runtime
            for i=1:length(timeInterval)
                configNow = interp1(tTask,stateTask(:,1:6),timeInterval(i)); % Interpolate using time, state, joint [1,6], current time
                %configNow(1) = configNow(1) + pi;
                P=a0+a1*timeInterval(i)+a2*timeInterval(i).^2+a3*timeInterval(i).^3; % Aquire position
                ...
                poseNow=TBS*TSI;% TBI=TBS*TSI;
                show(ur5_RBT,configNow,'PreservePlot',false,'Frames','off'); % use this to show ideal linear trajectory
                plot3(poseNow(1,4),poseNow(2,4),poseNow(3,4),'b.','MarkerSize',20); % Use negative values to account for MatLab plot error % taskSpaceMarker = 
                drawnow; % Draw trajectory iteratively
            end
        ...
        function moveLalt(P0, Pf, v_max, a_max)
            ...
            for i=1:length(timeInterval)
                ...
                plot3(poseNow(1,4)/1000,poseNow(2,4)/1000,poseNow(3,4)/1000,'b.','MarkerSize',20); % taskSpaceMarker = 
                drawnow; % Draw trajectory iteratively
            end
            %legend(taskSpaceMarker, {'Defined in Task-Space'}); % Add a legend
